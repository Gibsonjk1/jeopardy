...
function getCategoryIds() {
	async function ids() {
		let cat = await axios.get('http://jservice.io/api/categories?count=6');
		console.log(cat);
		return getCategory(cat.data[0].id);
	}
	ids();
}
...
async function fillTable() {
	return categories.clues[Math.floor(Math.random() * 5)];
}
